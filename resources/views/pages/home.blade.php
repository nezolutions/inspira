<div class="relative w-full min-h-screen bg-gray-800 bg-cover bg-no-repeat bg-center bg-fixed" 
    ...
    <div class="absolute inset-0 bg-gradient-to-b from-gray-700/60 via-gray-800/50 to-gray-900/90"></div>
    ...
    <div class="relative z-20 w-full min-h-screen flex flex-col items-center justify-center px-6">
        <div class="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 items-center gap-8">
            <div class="flex flex-col gap-4 text-center lg:text-start">
                ...
                <h2 class="text-4xl lg:text-7xl font-extrabold text-white drop-shadow-md tracking-wide">
                    {{ $judul[1] ?? 'INSPIRA' }} {{ $judul[2] ?? now()->format('Y') }}
                </h2>

                <!-- Subtitle -->
                <p class="text-lg lg:text-2xl text-gray-200 font-medium leading-relaxed">{!! $home->title !!}</p>
                ...
                <div class="flex items-center justify-center lg:justify-start">
                    <a href="{{ $app->register ?? '#' }}" 
                       class="group mt-6 inline-flex items-center gap-2 px-6 py-3 rounded-full text-lg font-bold uppercase tracking-wide text-white bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 shadow-lg shadow-red-800/40 transition-transform duration-300 hover:scale-105">
                        Register Now
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-5 h-5 group-hover:translate-x-1 transition">
        ...
        @auth
            <div class="relative top-12 w-fit z-20">
                <a href="{{ url('e/home') }}" 
                   class="flex items-center gap-2 px-4 py-2 text-white text-base rounded-lg bg-gray-800/90 border border-gray-700 hover:bg-gray-900 transition">
