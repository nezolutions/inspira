...
@auth
<form class="fixed w-full flex justify-center bottom-30 xl:bottom-18 z-50 select-none" method="POST" action="{{ route('set')}}">
    @csrf
    <button type="submit" class="border {{ $mtc->is_active ? 'bg-green-600 hover:bg-green-500 border-green-700' : 'bg-red-600 hover:bg-red-500 border-red-700' }} w-4/5 max-w-md flex items-center justify-between shadow-2xl rounded-md p-2 cursor-pointer transition">
        <span class="w-full font-semibold tracking-wide text-sm xl:text-lg text-center text-white">
            {{ $mtc->is_active ? 'Activate Website' : 'Deactivate Website' }}
        </span>
    </button>
</form>
    ...
    <div class="bg-gray-700 w-4/5 max-w-7xl flex items-center justify-between shadow-2xl border border-gray-600 rounded-md">
        ...
        <div class="flex items-center justify-between w-full xl:w-fit text-white text-sm xl:text-lg">
            <a href="{{ url('e/user')}}" class="flex items-center px-4 py-2 border-s border-transparent xl:border-gray-600 hover:bg-gray-600 rounded-s-sm xl:rounded-s-none gap-1 transition">
                ...
                <span class="hidden sm:inline">Account </span>Settings
            </a>
